/*
	- In SQLite, a unique index is an index that ensures the values 
	stored in a column or a set of columns are unique across the table.

	- In other words, no two rows can have the same values in the 
	indexed columns.
	...
	- In this syntax:
		+ First, specify the index name after the CREATE UNIQUE INDEX 
		keywords.
		+ Second, use the IF NOT EXISTS option to create an index 
		only if it does not exist.
		+ Third, provide the table name with which the unique index 
		is associated.
		+ Finally, list out one or more index columns of the table 
		inside the parentheses after the table name.
		
	** Note ** that SQLite considers NULLs different. It means you can 
	have multiple NULLs inside the columns of the unique index. 
*/
...
-- Fourth, insert another row into the contacts table with the email that 
-- already exists:
INSERT INTO contacts(name, email)
VALUES('John Smith', 'john@test.com');
...
INSERT INTO contacts(name, email) VALUES ('Alice', NULL);
INSERT INTO contacts(name, email) VALUES ('Bob', NULL);


-- Finally, query data from the contacts table:
SELECT * FROM contacts;
-- The output indicates that SQLite allows multiple NULLs in the column of 
-- the unique index.
...
VALUES (2024,1), (2024,2), (2024,3);

-- Fourth, retrieve data from the calendars table:
SELECT * FROM calendars;
-- The output indicates that SQLite uses the values in the year and 
-- month columns to evaluate the uniqueness.

-- Fifth, attempt to insert a year and month that already exists:
INSERT INTO calendars(year, month)
VALUES (2024,3); 
-- Error: Runtime error: UNIQUE constraint failed: 
-- calendars.year, calendars.month (19)

/*
	Summary
	- Use CREATE UNIQUE INDEX statement to create a unique index on one 
	or more columns of a table.
	- SQLite treats NULLs differently. It means you can have multiple NULLs 
